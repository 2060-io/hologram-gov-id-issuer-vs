...
public class VisionResource {
  ...
  @Operation(
      summary = "Link a media",
      description =
          "Link a media of type {type} with UUID {mediaId} to identity represented by token {token}")
  @APIResponses({
  ...
  public Response linkMedia(
      @PathParam(value = "token") UUID token, @PathParam(value = "mediaId") UUID mediaId) {
  ...
  @Operation(
      ...
      description =
          "List of mediaIds (UUID) of type {type} linked to identity represented by token {token}")
  @APIResponses({
    @APIResponse(responseCode = "400", description = "Check arguments or expired token"),
    @APIResponse(responseCode = "403", description = "Permission Denied."),
    @APIResponse(responseCode = "500", description = "Server error, please retry."),
  ...
  @Operation(
      ...
      description =
          "Notify a successful verification or capture for identity represented by token {token}.")
  @APIResponses({
    @APIResponse(responseCode = "400", description = "Check arguments or expired token."),
    @APIResponse(responseCode = "403", description = "Permission Denied."),
    @APIResponse(responseCode = "500", description = "Server error, please retry."),
  ...
  @Operation(
      ...
      description =
          "Notify a failed verification or capture for Identity represented by token {token}.")
  @APIResponses({
    @APIResponse(responseCode = "400", description = "Check arguments or expired token."),
    @APIResponse(responseCode = "403", description = "Permission Denied."),
    @APIResponse(responseCode = "500", description = "Server error, please retry."),
